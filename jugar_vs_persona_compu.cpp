...
void jugar_vs_persona_compu(int persona_o_computadora)
{
	bool gano_perdio = false; //controla si se gano o no el juego
	string era_la_palabra; //Aqui almaceno la palabra que en caso de perder le presento pa que sepa que era esa jeje 
	...
	do
	{
		...
		/********************************Controlar jugar vs persona o computadora****************************************************/
		if (persona_o_computadora == 0)
		{
	...
	} while (salir_volverAjugar(gano_perdio, era_la_palabra) != 0);
...
int salir_volverAjugar(bool victoria, string palabra)
{
	if (victoria == true)
	{
